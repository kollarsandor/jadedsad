      ...
      {result && (
        <div className="fold-result">
          ...
          <div className="result-info">
            <p><strong>Structure Generated Successfully!</strong></p>
            <p>Confidence Score: {result.confidence || '85.2%'}</p>
            <p>Structure ID: {result.structure_id || `structure_${Date.now()}`}</p>
          </div>
          
          <div className="result-actions">
            <h4>📁 Download Options</h4>
            <div className="download-links">
              <a href={`/api/download/structure_${result.structure_id || `structure_${Date.now()}`}.pdb`} 
                 download className="download-btn">
                📄 Download PDB File
              </a>
              <a href={`/api/download/structure_${result.structure_id || `structure_${Date.now()}`}.cif`} 
                 download className="download-btn">
                📋 Download mmCIF File
              </a>
              <a href={`/api/download/coords_${result.structure_id || `structure_${Date.now()}`}.json`} 
                 download className="download-btn">
                🔢 Download JSON Coordinates
              </a>
              <a href={`/api/download/pymol_${result.structure_id || `structure_${Date.now()}`}.pml`} 
                 download className="download-btn">
            ...
            <div className="viewer-actions">
              <a href={`/api/viewer/${result.structure_id || `structure_${Date.now()}`}`} 
                 target="_blank" className="viewer-btn">
                🔬 Open 3D Viewer
              </a>
              <a href={`/api/download/confidence_${result.structure_id || `structure_${Date.now()}`}.png`} 
                 download className="download-btn">
                📊 Download Confidence Map
              </a>
              <a href={`/api/download/pae_${result.structure_id || `structure_${Date.now()}`}.png`} 
                 download className="download-btn">
                📈 Download PAE Matrix
              </a>
            </div>
